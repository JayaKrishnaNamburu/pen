...
import { initBlockMap, validateDocument } from "../document";
...
describe("conflict resolution", () => {
	...
	describe("concurrent text edits in the same block", () => {
		...
		it("ordering is deterministic across all peers", () => {
			...
			const textA = docA.penDocument.blocks
				.get("b1")!
				.get("content") as Y.Text;
			const textB = docB.penDocument.blocks
				.get("b1")!
				.get("content") as Y.Text;
			const textC = docC.penDocument.blocks
				.get("b1")!
				.get("content") as Y.Text;
	...
	describe("concurrent block type/prop conversion", () => {
		it("converges to the same prop value via LWW", () => {
			...
			const propsA = docA.penDocument.blocks
				.get("b1")!
				.get("props") as Y.Map<unknown>;
			const propsB = docB.penDocument.blocks
				.get("b1")!
				.get("props") as Y.Map<unknown>;
	...
	describe("concurrent block deletion and content editing", () => {
		it("deletion wins — block is gone after merge", () => {
			...
			docA.ydoc.transact(() => {
				...
				docA.penDocument.blockOrder.push(["b1"]);
				const text = docA.penDocument.blocks
					.get("b1")!
					.get("content") as Y.Text;
	...
	describe("concurrent block reordering", () => {
		it("produces duplicate in blockOrder that normalization can fix", () => {
			...
			expect(docA.penDocument.blockOrder.toArray()).toEqual([
				"b3",
				"b1",
				"b2",
			]);
	...
	describe("schema version mismatch", () => {
		it("unknown block types are preserved after merge", () => {
			...
			const fromA = Y.encodeStateAsUpdate(
				docA.ydoc,
				Y.encodeStateVector(docB.ydoc),
			);
			...
			expect(docA.penDocument.blocks.get("b1")!.get("type")).toBe(
				"callout",
			);
			expect(docB.penDocument.blocks.get("b1")!.get("type")).toBe(
				"callout",
			);
		});
	});
