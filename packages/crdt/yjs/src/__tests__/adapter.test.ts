...
describe("adapter", () => {
	...
	describe("getAttributionRanges", () => {
		it("returns separate ranges for inserts from different clients", () => {
			...
			local.ydoc.transact(() => {
				...
				block.set("content", new Y.Text("hello"));
				local.penDocument.blocks.set("b1", block);
			});
			...
			expect(adapter.getAttributionRanges?.(local, "b1")).toEqual([
				{
					offset: 0,
					length: 5,
					clientId: local.ydoc.clientID,
				},
				{
					offset: 5,
					length: 6,
					clientId: remote.ydoc.clientID,
				},
			]);
		});
	});
