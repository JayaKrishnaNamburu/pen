...
export function initBlockMap(
	...
	const parentDoc = (blocks as Y.Map<Y.Map<unknown>> & { doc?: Y.Doc | null })
		.doc;
	...
	switch (contentType) {
		...
		case "subdocument":
			blockMap.set(
				SUBDOCUMENT,
				parentDoc
					? createYjsSubdocument(parentDoc)
					: new Y.Doc({ autoLoad: true }),
			);
			break;
		case "nested":
...
export function isYjsCRDTDocument(doc: unknown): doc is YjsCRDTDocument {
	return (
		doc != null &&
		typeof doc === "object" &&
		"ydoc" in doc &&
