...
`openAIToolCall` is the same grant for hosts that stream `executeTool` themselves (SSE and direct). It returns `{ ok: false }` without running the handler, or `{ ok: true, close }` after installing the write guard. Do not call `executeTool` unless it opened.
