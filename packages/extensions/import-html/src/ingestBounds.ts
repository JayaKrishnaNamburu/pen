...
/**
 * Ingest envelope (IOP5 / SEC4). Same numbers as
 * `@input/pen-import-markdown` and `@input/pen-import-json` — not
 * per-format values. Those packages each keep a local copy because the
 * shared-constant extract is out of this step's fence; the numbers must
 * stay identical so one document arriving by two routes has one limit.
 *
