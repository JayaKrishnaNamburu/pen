...
function renderProvenanceIntro(fixtures: PasteCorpusFixture[]): string {
  ...
  const missing = synthetic
    ...
    .join(", ");
  return `${fixtures.length - synthetic.length} of ${fixtures.length} sources are hand-captured. Still synthetic-until-capture: ${missing}. The replacement procedure is \`src/__tests__/pasteCorpus/CAPTURE.md\`.`;
}
