...
function readHostResolver(editor: Editor): ResolveSnapshotAuthor | undefined {
	return (editor.facet(snapshotsControllerFacet) as SnapshotsController | null)
		?.resolveAuthor;
}

function toVerifiedAuthor(
	identity: SnapshotAuthorIdentity,
): VerifiedSnapshotAuthor {
	return {
		verified: true,
		id: identity.id,
