...
import { getSnapshotsController, snapshotsExtension } from "../index";

describe("snapshotsExtension", () => {
	it("registers the history controller on the editor", () => {
		...
		expect(editor.facet(snapshotsControllerFacet)).toBeTruthy();
		expect(getSnapshotsController(editor)).toBeTruthy();
	});
	...
	it("restores a previous snapshot across all editors on the same document session", async () => {
		...
		const controller = getSnapshotsController(editorA)!;
		...
		expect(getSnapshotsController(editorA)).toBe(
			getSnapshotsController(editorB),
		);
		expect(getSnapshotsController(editorA)).toBeTruthy();
		expect(getSnapshotsController(editorB)).toBeTruthy();
	...
	it("shares one history controller per document scope", async () => {
		...
		expect(editorA.facet(snapshotsControllerFacet)).toBe(
			editorB.facet(snapshotsControllerFacet),
		);
		expect(getSnapshotsController(editorA)).toBe(
			getSnapshotsController(editorB),
		);
	...
	it("restores nested subdocument editors when the root scope snapshot is restored", async () => {
		...
		const controller = getSnapshotsController(rootEditor)!;
