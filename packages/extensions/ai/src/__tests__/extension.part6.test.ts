...
describe("aiExtension", () => {
					...
					model: {
						async *stream() {
							yield {
								...
								delta: turnOutputs[turnIndex] ?? "done",
							};
							yield { type: "done" as const };
							turnIndex += 1;
						},
	...
	it("undoes a server-authored result when the prompt has a newer UI-only snapshot", async () => {
		...
		expect((controller as any).handleInlineHistoryShortcut("undo")).toBe(
			true,
		);
		expect(editor.getBlock(blockId)?.textContent({ resolved: true })).toBe(
			"Hello world",
	...
	it("walks server-authored inline turn results one turn at a time", async () => {
		...
		const firstUndoTargetIndex = (
			controller as any
		)._resolveInlineHistoryTargetIndex("undo", { shortcutOnly: true });
		expect(
			(controller as any)._inlineHistory[firstUndoTargetIndex]
				?.sessions[0]?.turns,
		).toHaveLength(1);
		expect(
			(controller as any)._inlineHistory[firstUndoTargetIndex]
				?.sessions[0]?.turns[0]?.status,
		).toBe("review");
		expect((controller as any).handleInlineHistoryShortcut("undo")).toBe(
			true,
		);
		...
		expect((controller as any).handleInlineHistoryShortcut("redo")).toBe(
			true,
		);
		expect(editor.getBlock(blockId)?.textContent({ resolved: true })).toBe(
			"Hello there friend",
