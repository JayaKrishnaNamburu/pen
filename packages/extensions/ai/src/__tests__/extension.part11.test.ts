...
describe("aiExtension", () => {
	...
	it("reuses a leading empty placeholder for document-target bottom-chat writes", async () => {
		...
		expect(
			readBlockSuggestionMeta(editor.getBlock(placeholderBlockId))
				?.action,
		).toBe("delete-block");
	});
	...
	it("creates tables through markdown for bottom-chat document prompts", async () => {
		...
		expect(tables[0].as("table")?.tableCell(0, 0)?.textContent()).toBe(
			"Tier",
		);
		expect(tables[0].as("table")?.tableCell(0, 1)?.textContent()).toBe(
			"Price",
		);
