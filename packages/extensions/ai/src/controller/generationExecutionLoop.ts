...
export async function runGenerationLoop(
	...
	const result = await runAgenticLoop({
		...
		onTextDelta: (delta) => {
			const nextDelta =
				target.type === "block" && state.shouldTrimLeadingBlankBlockText
					? trimLeadingBlankBlockGenerationText(delta)
					: delta;
			if (state.shouldTrimLeadingBlankBlockText && nextDelta.length > 0) {
		...
		onStructuredData: (event) => {
			...
			if (context?.sessionId && sessionTurnId) {
				controller._updateSessionTurn(
					...
					sessionTurnId,
					{
						reviewItemIds: nextStructuredPreview.reviewItems.map(
							(item) => item.id,
						),
						structuredPreview: nextStructuredPreview,
					},
