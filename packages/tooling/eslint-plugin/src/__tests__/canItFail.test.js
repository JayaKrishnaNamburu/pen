...
describe("per-rule can-it-fail (write a violation, error by name)", () => {
	it("no-html-injection-sinks errors by name on innerHTML assignment", () => {
		expectRuleErrors(tsTester, "no-html-injection-sinks", rules["no-html-injection-sinks"], {
	...
	it("no-unescaped-markup-concat errors by name on interpolated markup", () => {
		expectRuleErrors(tsTester, "no-unescaped-markup-concat", rules["no-unescaped-markup-concat"], {
	...
	it("no-above-floor-api errors by name on bare structuredClone", () => {
		expectRuleErrors(tsTester, "no-above-floor-api", rules["no-above-floor-api"], {
	...
	it("no-bare-random-uuid errors by name on crypto.randomUUID, including a feature test", () => {
		expectRuleErrors(tsTester, "no-bare-random-uuid", rules["no-bare-random-uuid"], {
	...
	it("no-framework-free-modules-in-renderers errors by name on a leftover module", () => {
		expectRuleErrors(tsTester, "no-framework-free-modules-in-renderers", rules["no-framework-free-modules-in-renderers"], {
			code: "export function leftover() { return 1; }\n",
			filename: "packages/rendering/react/src/seeded-framework-free.ts",
			errors: [{ messageId: "frameworkFree" }],
		});
	});

	it("no-module-scope-browser-globals errors by name on document at module scope", () => {
		expectRuleErrors(tsTester, "no-module-scope-browser-globals", rules["no-module-scope-browser-globals"], {
	...
	it("no-user-facing-literals errors by name on button copy", () => {
		expectRuleErrors(jsxTester, "no-user-facing-literals", rules["no-user-facing-literals"], {
	...
	it("no-ascii-word-boundaries errors by name on a \b regex", () => {
		expectRuleErrors(tsTester, "no-ascii-word-boundaries", rules["no-ascii-word-boundaries"], {
	...
	it("no-bare-case-folding errors by name on toLowerCase in a matching path", () => {
		expectRuleErrors(tsTester, "no-bare-case-folding", rules["no-bare-case-folding"], {
	...
	it("no-implicit-locale errors by name on localeCompare without a locale", () => {
		expectRuleErrors(tsTester, "no-implicit-locale", rules["no-implicit-locale"], {
	...
	it("no-aria-hidden-visible errors by name on aria-hidden=true, and leaves setAttribute string true as the ARIA form", () => {
		expectRuleErrors(jsxTester, "no-aria-hidden-visible", rules["no-aria-hidden-visible"], {
		...
		expectRuleErrors(tsTester, "no-aria-hidden-visible", rules["no-aria-hidden-visible"], {
	...
	it("no-unstyled-focus errors by name on outline none", () => {
		expectRuleErrors(tsTester, "no-unstyled-focus", rules["no-unstyled-focus"], {
			code: 'export const style = { outline: "none" };\n',
			filename: "packages/rendering/react/src/seeded-unstyled-focus.ts",
			errors: [{ messageId: "outlineNone" }],
		});
	});

	it("no-v1-extension-fields errors by name on keyBindings", () => {
		expectRuleErrors(tsTester, "no-v1-extension-fields", rules["no-v1-extension-fields"], {
	...
	it("no-selection-timers errors by name on setTimeout in a selection module", () => {
		expectRuleErrors(tsTester, "no-selection-timers", rules["no-selection-timers"], {
			code: "setTimeout(() => {}, 0);\n",
			filename:
