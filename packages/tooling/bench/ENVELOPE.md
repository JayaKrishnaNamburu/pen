...
| `crdt.bench.ts` fork + merge | fork + merge 100-block document | Forks a 100-block Y.Doc, inserts FORK-MERGE-TOKEN on block-50 of the fork, then clocks merge into the target. Observation after the clock names block-50. | agrees | count-trusted; clock load-taken | count: target block-50 contains FORK-MERGE-TOKEN after merge. A skipped merge or a self-copy fails assertMergeTransferred |
...
| `scale3.peer-count.8` | SCALE3 peer-count 8 | enforced | remote-caret decorations !== 8 or a second Y.Doc appears | decorative | critical:true with ~13× slack (3.73ms → 50ms); peer count is decorations |
...
| `crdt.fork-merge-100` | CRDT fork + merge of a diverged 100-block document | enforced | assertMergeTransferred: target block-50 missing FORK-MERGE-TOKEN | record-only | empty-timer floor; count is the token on the named block |
