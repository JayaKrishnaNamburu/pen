...
| `scale3.remote-caret-count.8` | SCALE3 remote-caret-count 8 | enforced | remote-caret decorations !== 8 | gated | critical:true; 0.06ms median against a 2ms gate. Axis is caret decorations, not synced Y.Docs. N-peer scaling is unmeasured |
| `scale3.keystroke.realistic-stack` | SCALE3 realistic-stack keystroke clocks | record-only | synthetic gate compare only; no live p50 assertion | gated | critical:true; gates are 1–2ms on 0.04–0.06ms medians, set below the pre-SCALE2-fix 0.48/3.76ms so per-document commit work fails |
