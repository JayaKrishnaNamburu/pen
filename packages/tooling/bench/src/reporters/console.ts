...
export function reportConsole(
	...
	console.error(
		`  CH8 gate: median of ${results[0]?.iterations ?? 0} samples. P95 and Max are trend-only.`,
	);
	console.error(`  ${"=".repeat(nameWidth + 95)}\n`);

	console.error(
		`  ${"Benchmark".padEnd(nameWidth)}  ${"Avg (ms)".padStart(10)}  ${"Median (ms)".padStart(12)}  ${"P95 trend".padStart(10)}  ${"Min (ms)".padStart(10)}  ${"Max trend".padStart(10)}  ${"ops/s".padStart(10)}  ${"Target".padStart(10)}`,
	);
