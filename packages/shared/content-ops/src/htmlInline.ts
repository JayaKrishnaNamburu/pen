...
import type { InlineMark } from "./markdownTypes";
...
const inlineMarkMap: Record<string, string> = {
  strong: "bold",
  b: "bold",
  em: "italic",
  i: "italic",
...
export function collectInlineHtmlContent(html: string): {
  ...
  for (const child of doc.children ?? []) {
    walkHtmlNode(child as ParsedHtmlNode, ctx);
  }
...
function walkHtmlNode(node: ParsedHtmlNode, ctx: InlineContext): void {
  ...
  if (node.type === "tag" || node.type === "script" || node.type === "style") {
    ...
    const markType = inlineMarkMap[el.name];
    if (markType) {
