...
import { createDirectionCache, fingerprintDirectionInput } from "../cache";

describe("direction cache DIR1", () => {
	it("DIR1: fingerprint changes when text, props, or facet key change", () => {
		const base = fingerprintDirectionInput(
			"Hello",
			{ direction: "auto" },
			"ltr:0",
		);
	...
	it("DIR1: get misses after text/props change, invalidate, or clear", () => {
		...
		expect(cache.get("b1", "Hello", { direction: "auto" }, "ltr:0")).toBe(
			"ltr",
		);
		expect(
			cache.get("b1", "مرحبا", { direction: "auto" }, "ltr:0"),
		).toBeUndefined();
