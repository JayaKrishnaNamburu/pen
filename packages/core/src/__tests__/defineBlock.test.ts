...
describe("defineBlock", () => {
	...
	it("AX4: defineBlock().a11y attaches a frozen spec", () => {
		const schema = defineBlock("widget", { content: "none" }).a11y({
			label: (props) => String(props.title ?? "Widget"),
			roleDescription: "widget",
		});
		expect(typeof schema.a11y?.label).toBe("function");
		expect(schema.a11y?.roleDescription).toBe("widget");
		expect(Object.isFrozen(schema.a11y)).toBe(true);
		expect(
			typeof schema.a11y?.label === "function"
				? schema.a11y.label({ title: "Chart" })
				: "",
		).toBe("Chart");
	});
});
