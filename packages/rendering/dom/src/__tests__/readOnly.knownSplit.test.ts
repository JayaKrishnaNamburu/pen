...
function createBareEditor(readOnlyFacetValue?: boolean): Editor {
	return createEditor({
...
describe("mountEditor pen.readOnly vs readonly prop (known split, owner decision pending)", () => {
	...
	it("readOnly facet announces aria-readonly and still accepts typing (known split, owner decision pending)", () => {
	...
	it("readonly prop announces aria-readonly and declines typing (known split, owner decision pending)", () => {
	...
	it("readOnly facet plus readonly prop: prop wins for typing, both set aria-readonly (known split, owner decision pending)", () => {
		const editor = createBareEditor(true);
		const mounted = mount(editor, true);
		expect(editor.facet(readOnlyFacet)).toBe(true);
		expect(mounted.root.getAttribute("aria-readonly")).toBe("true");
