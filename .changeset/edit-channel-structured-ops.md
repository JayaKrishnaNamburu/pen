...
Add `format_text` and `set_block_props` to the `edit_document` tool so marks and in-place type/prop changes travel as structured operations (`spec/packages/extensions/ai.md`, EC18). Markdown cannot carry `textColor` / `backgroundColor` / heading level, and `replace_blocks` reassigns identity on a 1-to-1 type change — the two holes the Wave 0 corpus recorded.
