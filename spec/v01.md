...
- Enter to split block. Backspace at the start of an empty exit-style block (list items, blockquotes, callouts) converts it to a paragraph; otherwise Backspace at block start merges with the previous compatible block.
...
- **`@pen/mcp`** — MCP server (stdio, SSE, streamable HTTP). Exposes Pen's tools to external MCP clients (Claude Desktop, Cursor, etc.). `stdio` is process-scoped and started directly by Pen. SSE and streamable HTTP are request-scoped and are embedded through host-framework request handlers rather than the generic server lifecycle. The MCP client drives the conversation; Pen only executes tool calls. This is a fundamentally different pattern — bidirectional protocol, not HTTP API.
...
All `DiagnosticEvent` emissions (see `wave-00-foundation-types.md`, `PenEventMap`) must include a stable `code` field. Codes enable tooling, documentation linking, and filtering without relying on message string matching.
...
| `TOOL`        | Tool server, tool execution, MCP bridge                       | `@pen/document-ops`, `@pen/mcp` | 001–099    |
...
| **Tier 2 — Default extensions**   | `@pen/document-ops`, `@pen/delta-stream`, `@pen/undo`, `@pen/schema-default`, `@pen/import-*`, `@pen/export-*`, `@pen/mcp`                                                              | Stable public API. Breaking changes require a major version bump. Internal implementation details may change in minor versions.                                                             |
| **Tier 3 — Ecosystem**            | `@pen/ai`, `@pen/collaboration`, `@pen/history`, `@pen/search`, `@pen/input-rules`, `@pen/layout`, `@pen/apps`, `@pen/execution`, `@pen/skills`, `@pen/branch`, `@pen/auth`, `@pen/cli` | Public API is stable within a major version. May evolve faster than Tier 1/2 during early milestones.                                                                                       |
...
const editor = createEditor({
  extensions: [mcpProvider({ url: "http://localhost:3001/mcp" })],
});
```
