...
rg -o '^[-*]\s+([A-Z]+)[0-9]+\s*[.—–]' -r '$1' spec --glob '*.md' | sort -u
...
| UC     | `spec/rules/ai.md`                   | live        | one edit channel. Reserved 2026-08-26 with the v5 train; folded here when it landed, IDs unchanged                                                  |
| RS     | `spec/rules/ai.md`                   | live        | review surface. Not a member of the live `R` family — `RS1` is not `R1`                                                                             |
| FE     | `spec/rules/dom.md`                  | live        | field-editor spine. Not a member of the reserved `F` token — `FE1` is not `F1`                                                                      |
| HB     | `spec/rules/host.md`                 | live        | hosts and bindings. A separate family from `HOST` in the same file — `HB1` is not `HOST1`                                                           |
| WA     | `spec/charter/working-agreements.md` | record-only | working agreements. `PROCESS_PREFIXES` in `coverage-rules.mjs` — excluded from derivation, not a test-name obligation                               |
...
| W      | (none — token reserved)              | record-only | was the v2 adoption list W1–W9. Token reserved                                                                                                      |
