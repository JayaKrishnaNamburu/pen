...
- `@input/pen-ingest` and `@input/pen-markdown` depend on `@input/pen-core`. Core no longer depends on those packages. Import and profile-policy helpers export from core; ingest keeps parse/write construction.
- `defaultPreset()` is the batteries-included composition, and `@input/pen`'s `createEditor()` applies it when no `preset` is passed. Core's bare `createEditor()` is the apply pipeline only: no schema, no extensions.
