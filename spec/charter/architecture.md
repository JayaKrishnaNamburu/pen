...
- `@input/pen-content-ops` and `@input/pen-markdown-serialization` depend on `@input/pen-core`. Core no longer depends on those packages. Import and profile-policy helpers export from core; content-ops keeps parse/write construction.
