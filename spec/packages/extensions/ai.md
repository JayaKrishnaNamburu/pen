...
- Suggestion helpers such as `acceptSuggestion()`, `rejectSuggestion()`, `readAllSuggestions()`, and suggest-mode interceptors. Runtime review items are `PersistentSuggestion` / `PersistentBlockSuggestion` (narrower `BlockSuggestion` on `@input/pen-types` is the contract-layer subset).
- Review styling re-exports on the main entry: `REVIEW_SURFACE_CLASSES` and `REVIEW_SURFACE_CUSTOM_PROPERTIES` from `@input/pen-types`. The default sheet is `PEN_REVIEW_STYLESHEET` on `@input/pen-dom` (RS4); this package cannot re-export it without depending on a renderer (API1).
...
Workspace package at version `0.1.5`; intended usage is current-state but still evolving. This is one of the most ambitious packages in the workspace and should be treated as a large extension surface rather than a minimal helper package.
