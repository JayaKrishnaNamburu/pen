...
- `useSlashMenu`'s `confirm` never leaves the trigger in the document. The trigger range is the one `getSlashTarget` matched — offset 0 through the caret, `/` and query together — and confirm deletes it in the same `undoGroup: true` apply that installs the chosen block. What is left over decides the shape of that apply: nothing left means the trigger was the whole block, so the block is converted in place and the author gets the block they asked for where they asked for it; text left over (a caret parked mid-word, or a confirm with no trigger at all, which is how a host-supplied `SlashMenu.Input` drives the hook) means the leftover belongs to the author, so it keeps its block and the chosen type is inserted as a sibling. Leaving `/head` behind was AX3-visible, not only untidy: `getSlashTarget` matches any block whose text starts with `/`, so the listbox reopened whenever selection returned to the leftover paragraph. Confirm reads the trigger from the live document rather than from `state.target`, because `setQuery` moves `target.endOffset` to track an input the document never received.
...
Workspace package at version `0.1.0`; intended usage is current-state but still evolving. This is still the main renderer the repo documents and validates most thoroughly.
