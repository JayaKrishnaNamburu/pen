...
- `InlineAtomPortalLayer` mounts each host `inlineAtomRenderers` entry with `createElement`, so a renderer that calls hooks keeps a stable hook order when targets appear after the first paint. Calling the renderer as a plain function from the layer would register those hooks on the layer and trip React's hooks-order warning.
...
Workspace package at version `0.1.4`; intended usage is current-state but still evolving. This is still the main renderer the repo documents and validates most thoroughly.
