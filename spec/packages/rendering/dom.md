...
- `mountEditor({ readonly: true })` declines pointer activation and sets `data-readonly`. `pen.readOnly` is read only for `aria-readonly`. The facet does not decline typing. That split is an open owner decision.
