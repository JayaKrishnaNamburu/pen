...
This RFC is superseded by `spec/flow-mode-rfc.md`.
...
- `spec/wave-03-editor-core.md` and `packages/types/src/types/editor.ts` center document state around `blockOrder`
- `packages/types/src/types/selection.ts` and `packages/types/src/types/ops.ts` address text through `{ blockId, offset }`
- `spec/wave-05-react-rendering.md` and `packages/rendering/react/src/primitives/editor/content.tsx` render the editor as a block list with a shared field editor layered on top
