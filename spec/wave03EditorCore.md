...
├── tool-runtime.ts            ToolRuntimeImpl
...
@pen/delta-stream      → @pen/document-ops (ToolRuntime, type-only)
```
...
**Extension-managed slots.** Extensions that need to expose state to other extensions (e.g. `@pen/undo` exposes the undo manager, `@pen/document-ops` exposes the tool runtime, `@pen/delta-stream` exposes the streaming target) use `editor.internals.setSlot(key, value)` during activation and `editor.internals.getSlot<T>(key)` for retrieval. This replaces the `(editor as any)._toolRuntime = ...` monkey-patching pattern with a typed, collision-safe registry.

**Slot key conventions.** Each extension owns a namespace: `undo:manager`, `document-ops:toolRuntime`, `delta-stream:target`. Keys are plain strings — the convention prevents collisions without runtime enforcement.
...
Default extension for block CRUD (Spec Section 14.1). Registers tools with the document tool runtime.
...
### Module: `tool-runtime.ts` — ToolRuntimeImpl
...
The tool runtime is a simple registry-plus-executor. Input validation against `inputSchema` is intentionally lightweight in M0 — the schema is provided to the model via `ToolSchema` and most LLM providers validate outputs against the schema. Full runtime-side validation can be added via a validator extension.
...
export function documentOpsExtension(): Extension {
  ...
  return defineExtension({
    ...
    activateClient: async (ctx) => {
      ...
      // Expose tool runtime for @pen/delta-stream and @pen/ai
      ctx.editor.internals.setSlot("document-ops:toolRuntime", toolRuntime);
    },
...
export async function processStream(
  ...
  const toolRuntime =
    editor.internals.getSlot<ToolRuntimeImpl>("document-ops:toolRuntime") ?? null;
...
- **`@pen/document-ops` tools are the LLM's interface.** The LLM never calls `editor.apply()` directly. It calls tools like `write_document`, `insert_block`, and `delete_block` through the `ToolRuntime`. These tools validate inputs, resolve positions, and translate between LLM-friendly formats (Markdown, JSON) and `DocumentOp` operations. This indirection provides a stable API contract, schema validation at the tool boundary, and streaming support for text content.
