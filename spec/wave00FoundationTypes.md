...
10. **`ToolRegistry` / `ToolRuntime` / `ToolDefinition` interfaces missing.** `ToolContext` exists but the registry/runtime split and `ToolDefinition` do not.
...
│   ├── tools.ts            ToolRegistry, ToolRuntime, ToolDefinition, ToolContext, ModelAdapter, ModelStreamEvent, ModelMessage, ToolSchema
...
**Add `ToolRegistry` and `ToolRuntime` interfaces** (Spec Section 13.2):
