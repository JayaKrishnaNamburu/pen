...
This RFC translates the existing selection and field-editor requirements in `spec/v01.md`, `spec/wave03EditorCore.md`, and `spec/wave05ReactRendering.md` into an implementation architecture for cross-block selection and editing.

It also serves as a core enabling spec for `spec/flowModeRfc.md`: cross-block text editing is the main way Pen's block-native model can feel continuous for writing-first documents without introducing a second document architecture.
...
## AI Inline Edit Requirements

Cross-block selection is also a first-class target for AI rewrite flows.

- Inline AI edit must target canonical `DocumentRange` semantics, not DOM-only geometry.
- A follow-up prompt must re-resolve against a tracked range snapshot for the active inline-edit session, even if the visible selection has temporarily collapsed.
- Same-block range rewrites may stream reviewable suggestion text incrementally.
- Cross-block range rewrites must still resolve into reviewable persistent suggestions, using the same range replacement semantics as core selection replacement.
- The inline AI card may be rendered in document flow, but it is presentation-only. The tracked selection and emitted suggestions remain the semantic source of truth.
