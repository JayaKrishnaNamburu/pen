...
### Inline Edit Card Semantics

Selection-scoped inline edit is a session-oriented review flow, not a one-shot toolbar action.

- Opening inline edit from the selection toolbar or shortcut creates or focuses an `inline-edit` AI session.
- Each submitted prompt becomes a distinct session turn with its own prompt text, generation id, tracked target snapshot, pending suggestion ids, pending review item ids, and status.
- The inline card stays attached to the selected document region while the session is active. Accepting or rejecting the inline edit resolves that turn's review state and closes the card. Pressing Escape rejects the inline edit and closes the card.
- Session-level pending ids are derived from the union of unresolved turn-level ids so the controller can still expose aggregate state for convenience.
- Persistent suggestions remain the only review model for inline edits. The inline card is UI-only and must never become persisted document content.
...
The core model-tool-model loop. Accepts a `ModelAdapter`, calls the model, parses tool calls, executes via `ToolRuntime`, feeds results back, and repeats until the model produces text without tool calls or hits the step limit.

```typescript
import type { ModelAdapter, ToolRuntime, Editor, Unsubscribe, ModelMessage, StreamingTarget, PenStreamPart } from '@pen/types';
...
export async function runAgenticLoop(
  ...
  const {
    model, editor, toolRuntime, prompt, blockId,
  ...
  const tools = toolRuntime.listTools().map(t => ({
    name: t.name,
  ...
  while (stepIndex < maxSteps) {
    ...
    for (const toolCall of pendingToolCalls) {
      ...
      try {
        const result = toolRuntime.executeTool(toolCall.toolName, toolCall.input, ctx);
        let output: unknown;
...
- **Tool result buffering.** `AsyncIterable` results from tools are buffered into arrays, matching the native `@pen/ai-tools` execution helpers from Wave 6.
