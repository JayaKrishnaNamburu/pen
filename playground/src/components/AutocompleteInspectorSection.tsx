...
type AutocompleteInspectorSectionProps = {
	editor: Editor;
...
export function AutocompleteInspectorSection({
	editor,
	autocompleteSettings,
	...
	onAutocompleteAcceptanceStrategyChange,
	onAutocompleteBlockPolicyChange,
}: AutocompleteInspectorSectionProps) {
	...
	const autocompleteMetricItems = [
		...
		},
		{
			...
			value:
				autocomplete.state.diagnostics.lastPolicyInvalidationStage ??
				"None",
		},
	];
	...
	const providerTimingItems = autocomplete.state.providerTimings.map(
		(timing) => (
	...
	const blockPolicyItems = [
		{
			label: "Code blocks",
			configured: formatToggleValue(
				configuredBlockPolicy.allowInCodeBlocks,
			),
		...
		},
		{
			label: "Allowed types",
			configured:
				formatBlockTypeList(configuredBlockPolicy.allowedBlockTypes) ||
				"Any",
			active:
				formatBlockTypeList(activeBlockPolicy.allowedBlockTypes) ||
				"Any",
		},
		{
			label: "Denied types",
			configured:
				formatBlockTypeList(configuredBlockPolicy.deniedBlockTypes) ||
				"None",
			active:
				formatBlockTypeList(activeBlockPolicy.deniedBlockTypes) ||
				"None",
		},
	];
	...
	return (
		<section className="inspector-section">
			...
			<div className="inspector-controls">
				...
				<label className="inspector-range-row">
					...
					<select
						...
						onChange={(event) =>
							onAutocompleteAcceptanceStrategyChange(
								event.target
									.value as AutocompleteAcceptanceStrategy,
							)
						}
					>
				...
				<label className="inspector-toggle-row">
					...
					<input
						type="checkbox"
						checked={
							autocompleteSettings.blockPolicy
				...
				<label className="inspector-toggle-row">
					...
					<input
						type="checkbox"
						checked={
							autocompleteSettings.blockPolicy.allowInTables ??
							false
						}
						onChange={(event) =>
							onAutocompleteBlockPolicyChange({
				...
				<label className="inspector-range-row">
					...
					<input
						...
						onChange={(event) =>
							onAutocompleteBlockPolicyChange({
								allowedBlockTypes: parseBlockTypeList(
									event.target.value,
								),
							})
						}
			...
			<div className="inspector-provider-list-wrap">
				...
				<ul className="inspector-provider-list">{blockPolicyRows}</ul>
			</div>
			{providerItems.length > 0 ? (
				<div className="inspector-provider-list-wrap">
					<h6 className="inspector-subtitle">Providers</h6>
					<ul className="inspector-provider-list">{providerItems}</ul>
				</div>
			) : null}
