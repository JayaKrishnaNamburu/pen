...
with the rest of the work. Private workspace packages (docs, playground,
examples, conformance, the ESLint plugin) are ignored by versioning and do
...
run before `changeset publish`. The GitHub release workflow versions the
train (a Version Packages pull request) and does not publish; after that
PR merges, publish from a maintainer terminal with `pnpm release`
(`changeset publish`). Tag the train with `vX.Y.Z` after a successful
publish. Per-package `CHANGELOG.md` files are generated by
